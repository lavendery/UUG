...
def generate(tokenizer, question, generation_config, model, gt_target_audio_ids, audio_tokens, gt_semantic_tokens, spk_emb=None):
    ...
    stop_words_ids = [torch.tensor([tokenizer.eos_token_id]).cuda(), torch.tensor([32012]).cuda()] # 32012是audio的end token: </audio>
...
if __name__ == "__main__":
    ...
    parser.add_argument("--tts-gen-dir", type=str, required=True, help="Path to the result of asr prediction")
    args = parser.parse_args()
    os.makedirs(args.tts_gen_dir, exist_ok=True)
    ...
    for dataset_path in all_dataset_path:
        ...
        for sample in dataset:
            try:
                ...
                question = tokenizer.bos_token + system_message + s_token + " " + text + sep + e_token + ' '
                gt_semantic_tokens = [item + 32013 for item in audio_ids]
                ...
                save_path = os.path.join(args.tts_gen_dir, file_name + '.wav')
