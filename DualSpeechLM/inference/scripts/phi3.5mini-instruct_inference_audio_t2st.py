...
if __name__ == "__main__":
    ...
    parser.add_argument("--t2st-gen-dir", type=str, required=True, help="Path to the result of asr prediction")
    args = parser.parse_args()
    os.makedirs(args.t2st_gen_dir, exist_ok=True)
    ...
    for dataset_path in all_dataset_path:
        ...
        for sample in dataset:
            try:
                ...
                system_message = "Please translate the " + data['metadata']['source_language'] + " text into " + data['metadata']['target_language'] + " speech. "

                question = tokenizer.bos_token + system_message + s_token + " " + input_text + sep + e_token + ' '
                answer_audio_ids = data['answer_ids']

                gt_semantic_tokens = [item + 32013 for item in answer_audio_ids]
                ...
                save_path = os.path.join(args.t2st_gen_dir, file_name + '.wav')
                decode_image_text(generate_ids, mywav_tokenizer, save_path)
                print(file_id, text)
                print("input_text: ", data['input_text'])
                cnt += 1
