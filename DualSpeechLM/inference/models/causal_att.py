...
class AnyGPT(nn.Module):
    ...
    def compute_accuracy(self, lprobs, target, ignore_id=4096):
        n_correct = torch.sum(
    ...
    @torch.no_grad()
    def generate(self, condition, mask, generated_idx, max_length, end_token=None, temperature=1.0, top_k=None, top_p=None):
        """
        condition: (batch_size, condition_length, embedding_dim)
        mask: (batch_size, condition_length)
        max_length
        temperature
        top_k
        top_p
        ...
        for _ in range(max_length):
            logits, _, _ = self(condition, mask, generated_idx, None)
