...
class USTokenizer(nn.Module):
    ...
    def tokenize(self, wav_path):
        wav_processor = WhisperFeatureExtractor.from_pretrained("checkpoint/openai/whisper-medium")
        ...
        spectrogram = whisper_extractor["input_features"].to(self.device)
        raw_wav = torch.from_numpy(audio).unsqueeze(0).to(self.device)
        audio_padding_mask = torch.zeros(len(audio), dtype=torch.bool).unsqueeze(0).to(self.device)

        speech_embeds, speech_atts, commitment_loss, perplexity, recon_loss, USToken = self.encode_speech(spectrogram=spectrogram, raw_wav=raw_wav, audio_padding_mask=audio_padding_mask, return_USToken=True)
        indices = USToken.squeeze()
        return indices
