...
class Encoder(nn.Module):
    def __init__(
        ...
        if attn_window_size is not None:
            layers += [LocalMHA(dim=d_model, window_size=attn_window_size) for i in range(att_layer)]
...
class Decoder(nn.Module):
    def __init__(
        ...
        if attn_window_size is not None:
            layers += [LocalMHA(dim=channels, window_size=attn_window_size) for i in range(att_layer)]
        # if attn_window_size is not None:
