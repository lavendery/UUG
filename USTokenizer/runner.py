...
from optims import get_optimizer, get_optimizer_low_lr_whisper, LinearWarmupCosineLRScheduler
...
class Runner:
    def __init__(self, cfg, model, datasets, job_id):
        ...
        self.checkpoint_interval = self.config.config.run.checkpoint_interval
        self.keep_last_checkpoints = self.config.config.run.keep_last_checkpoints
        ...
        # set the lr of whisper to lr * 0.1
        self.optimizer = get_optimizer_low_lr_whisper(self.model, self.config.config.run.optims)
    ...
    def train_epoch(self, epoch):
        ...
        metric_logger.add_meter("whisper_lr", SmoothedValue(window_size=1, fmt="{value:.6f}"))
        metric_logger.add_meter("llama_loss", SmoothedValue(window_size=1, fmt="{value:.4f}"))
        metric_logger.add_meter("commitment_loss", SmoothedValue(window_size=1, fmt="{value:.4f}"))
        metric_logger.add_meter("recon_loss", SmoothedValue(window_size=1, fmt="{value:.4f}"))
        metric_logger.add_meter("perplexity", SmoothedValue(window_size=1, fmt="{value:.4f}"))
        ...
        for i in metric_logger.log_every(range(self.iters_per_epoch), self.config.config.run.log_freq, header=header, logger=self.log_writter, start_step=epoch*self.iters_per_epoch):
            ...
            with torch.cuda.amp.autocast(enabled=self.use_amp):
                model_dict = self.model(samples)
                loss = model_dict["loss"]
                llama_loss = model_dict["llama_loss"]
                commitment_loss = model_dict["commitment_loss"]
                recon_loss = model_dict["recon_loss"]
            ...
            metric_logger.update(whisper_lr=self.optimizer.param_groups[2]["lr"])
            metric_logger.update(llama_loss=llama_loss.item())
            metric_logger.update(commitment_loss=commitment_loss.item())
    ...
    def save_checkpoint(self, cur_epoch, is_best=False):
        ...
        """
        # save best checkpoint without clean up
        if is_best:
            ...
            logging.info("Saving BEST checkpoint at epoch {} to {}.".format(cur_epoch, best_path))
        ...
        regular_path = os.path.join(self.output_dir, f"checkpoint_epoch_{cur_epoch}.pth")
        logging.info("Saving checkpoint at epoch {} to {}.".format(cur_epoch, regular_path))
        torch.save(save_obj, regular_path)

        # clean up old checkpoints 
        checkpoints = sorted(
            [f for f in os.listdir(self.output_dir) if f.startswith("checkpoint_epoch_")],
            key=lambda x: int(x.split("_")[2].split(".")[0]),
            reverse=True
        )
        if len(checkpoints) > self.keep_last_checkpoints:
            for old_checkpoint in checkpoints[self.keep_last_checkpoints:]:
